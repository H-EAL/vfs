    ...
    protected:
        ...
        static bool create_directory(const path &dirPath)
        {
            if (CreateDirectory(dirPath.c_str(), nullptr) == FALSE)
            {
                ...
                vfs_errorf("CreateDirectory({}) returned error: {}", dirPath.str(), get_last_error_as_string(errorCode));
                return false;
            }
            return true;
        }
        ...
        static bool delete_directory(const path &dirPath)
        {
            ...
            {
                const auto errorCode = GetLastError();
                vfs_errorf("RemoveDirectory({}) returned error: {}", dirPath.str(), get_last_error_as_string(errorCode));
                return false;
            }
            return true;
        }
        ...
        static void scan(const path &dirPath, std::vector<_Dir> &subDirectories, std::vector<path> &files)
        {
            ...
            {
                const auto errorCode = GetLastError();
                vfs_errorf("FindFirstFile({}) returned error: {}", dirPath.str(), get_last_error_as_string(errorCode));
                return;
            }
            ...
            {
                vfs_errorf("FindNextFile({}) returned error: {}", dirPath.str(), get_last_error_as_string(errorCode));
            }
